...
def create_fallback_summary(workspace_dir, env_vars, workflows_executed):
    ...
    summary_parts.append(f"Review completed with warnings. Some workflows may have encountered issues.")
    summary_parts.append("")
    ...
    summary_parts.append("---")
    summary_parts.append("")
    summary_parts.append(f"*🤖 Generated by CrewAI Router System | ⏱️ {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}*")
...
def generate_cost_breakdown():
    ...
    try:
        ...
        for i, call in enumerate(calls, 1):
            ...
            if call['duration'] > 0:
                speed = (call['tokens_in'] + call['tokens_out']) / call['duration']
                speed_str = f"{speed:.1f} tok/s"
            else:
                speed_str = "N/A"
            
            lines.append(f"| #{i} | {model_short} | {input_tokens} | {output_tokens} | {cost} | {speed_str} |")
        
        # Add totals row
        lines.append(f"| **TOTAL** | **{summary['total_calls']} calls** | **{summary['total_tokens_in']:,}** | **{summary['total_tokens_out']:,}** | **${summary['total_cost']:.6f}** | **{summary['total_duration']:.1f}s** |")
        
        lines.append("")
        lines.append(f"**Total Tokens**: {summary['total_tokens']:,} | **Avg Speed**: {summary['total_tokens']/summary['total_duration']:.1f} tok/s")
        lines.append("")
