...
def find_files_importing(repo_path: str, target_modules: Set[str]) -> List[str]:
    ...
    try:
        for root, dirs, files in os.walk(repo_path):
            ...
            dirs[:] = [
                d for d in dirs
                if d not in ['.git', '.crewai', 'node_modules', '__pycache__', '.venv', 'dist', 'build']
            ]
...
def RelatedFilesTool(changed_files: List[str], repository: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Found {len(related['related_files'])} related files for {len(changed_files)} changed files")
        return related
